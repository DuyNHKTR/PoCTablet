...
CUSTOM_ANCHORS = np.array(
    [
        [[2.5, 2.625], [3.0, 3.25], [3.625, 4.0]],
        [[3.4375, 3.8125], [4.6875, 5.4375], [6.5625, 8.5625]],
        [[6.5625, 8.781199932098389], [9.218800067901611, 11.375], [12.468999862670898, 15.531000137329102]],
    ],
    dtype="float32",
)
...
class BaseDetector:
    ...
    def detect_in_folder(self, data_path, max_output_size=15, iou_threshold=0.45, score_threshold=0.25):
        ...
        if len(imms) == 0:
            imms = glob(os.path.join(data_path, "*"))
            use_class = False
        #dest_path = "D:/Study/Kotora/face-detection-cropping/data/raw/Data_26-06-23_yolo"

        for imm in tqdm(imms, "Detecting"):
            a, b, c, nimages = self.detect_in_image(imm, max_output_size, iou_threshold, score_threshold, image_format="RGB")
...
class YoloV5FaceDetector(BaseDetector):
    ...
    def __init__(self, model_path=DEFAULT_DETECTOR, anchors=CUSTOM_ANCHORS, strides=DEFAULT_STRIDES):
...
# Get the current directory of function.py
current_dir = os.path.dirname(__file__)
# Navigate back one folder
parent_dir = os.path.abspath(os.path.join(current_dir, os.pardir))
path_img = os.path.join(parent_dir, "static/images/face_img/Quan2.jpg")
path_save =  os.path.join(parent_dir, "static/images/face_img/Quan.jpg")
data = YoloV5FaceDetector().detect_in_image(path_img)
image_rgb = cv2.cvtColor(data[3][0], cv2.COLOR_BGR2RGB)
cv2.imwrite(path_save,image_rgb)
